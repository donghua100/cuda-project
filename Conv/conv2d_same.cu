...
void cudnn_conv_2d(T *N, T *M, T *P, int ld) {
    ...
    int padA[] = {MaskWidth/2, MaskWidth/2};
...
int main()
{
    ...
    Ph = (float *)malloc(sizeof(float) * num);
    Phnn = (float *)malloc(sizeof(float) * num);
    ...
    CUDA_CHECK(cudaMallocPitch((void **)&Pd, &pitch, sizeof(float)*width, height));
    // cudaMalloc((void **)&Pd, sizeof(float)*width*height);
    CUDA_CHECK(cudaMallocPitch((void **)&Pdnn, &pitch_dnn, sizeof(float)*width, height));
    ...
    CUDA_CHECK(cudaMemcpy2D(Ph, sizeof(float)*width, Pd, pitch, sizeof(float)*width, height, cudaMemcpyDeviceToHost));
    // cudaMemcpy(Ph, Pd, sizeof(float)*num, cudaMemcpyDeviceToHost);
    cudaMemcpy2D(Phnn, sizeof(float)*width, Pdnn, pitch_dnn, sizeof(float)*width, height, cudaMemcpyDeviceToHost);
