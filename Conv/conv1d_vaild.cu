...
__global__ void conv_1d_basic(T *N, T *M, T *P) {
    ...
    // printf("%d\n", idx);
    if (idx < Width - MaskWidth + 1) P[idx] = val;
    // P[idx] = idx;
...
__global__ void conv_1d_share(T *N, T *P) {
    ...
    Nds[tid] = N[idx];
    if (tid < MaskWidth - 1) Nds[blockDim.x + tid] = N[right];
    __syncthreads();
    float val = 0;
    for (int i = 0; i < MaskWidth; i++) {
        val += Nds[tid + i]*M[i];
...
int main()
{
    ...
    float *Nh, *Mh, *Ph, *Phnn;
    constexpr int out_size = Width - MaskWidth + 1;
    Nh = (float *)malloc(sizeof(float) * Width);
    Mh = (float *)malloc(sizeof(float) * MaskWidth);
    Ph = (float *)malloc(sizeof(float) * out_size);
    Phnn = (float *)malloc(sizeof(float) * out_size);
    ...
    cudaMalloc((void **)&Pd, sizeof(float)* out_size);
    cudaMalloc((void **)&Pdnn, sizeof(float)*out_size);
    ...
    cudaMemcpy(Ph, Pd, sizeof(float)*out_size, cudaMemcpyDeviceToHost);
    cudaMemcpy(Phnn, Pdnn, sizeof(float)*out_size, cudaMemcpyDeviceToHost);
    compare<float, out_size>(Ph, Phnn);
    printarr<32>(Ph);
    printarr<32>(Phnn);
